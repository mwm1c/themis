	...
	int link_change, u64 *curr_tail)
{
	...
	/* wwb add 2020-6-27 */
	u64 block_nums = get_block_number_by_virt(sb,(unsigned long)(entry->name));
	log_block_write_count_increase(block_nums);
	//printk("WWB: nova_append_dir_inode_entry block number is %lu \n", block_nums);
	/* wwb end 2020-6-27 */
	...
	struct nova_inode *pi, u64 self_ino, u64 parent_ino)
{
	...
	nova_update_tail_wwb(sb, pi, curr_p);
