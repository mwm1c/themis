...
/* wwb add 2020-07-06 */
static inline struct nova_inode *original_nova_get_inode(struct super_block *sb,
	struct inode *inode)
{
	struct nova_inode_info *si = NOVA_I(inode);
	struct nova_inode_info_header *sih = &si->header;
...
/* 对于预留的那些inode，在已知pi_addr时获取inode的方式 */
static inline struct nova_inode *original_nova_get_inode_by_piaddr(struct super_block *sb, 
unsigned long pi_addr)
{
	return (struct nova_inode *)nova_get_block(sb, pi_addr);
}
	...
	struct inode *inode)
{
	u64 pi_addr;
	struct nova_inode * pi;
	int ret = nova_get_inode_address(sb, inode->i_ino, &pi_addr, 0);
	if (ret) {
		printk("WWB: get inode address error \n");
		return NULL;
	}

	/* wwb add 2020-07-06 */
	//if(inode->i_ino <= 1)
	if(inode->i_ino < NOVA_NORMAL_INODE_START)
	{
	...
	if(inode->i_ino != pi->nova_ino)
	{
		printk("nova_get_inode fatel error inode->i_ino is %lu , pi->nova_ino is %llu \n", \
			inode->i_ino, pi->nova_ino);
	}
...
static inline struct nova_inode *nova_get_inode_by_ino_wwb(struct super_block *sb,
	u64 ino)
{
	u64 pi_addr;
	struct nova_inode * pi;
	int ret = nova_get_inode_address(sb, ino, &pi_addr, 0);
	if (ret) {
		printk("WWB: get inode address error \n");
		return NULL;
	}
	...
	if(ino != pi->nova_ino)
	{
		printk("nova_get_inode_by_ino_wwb fatel error ino is %llu , pi->nova_ino is %llu \n", \
			ino, pi->nova_ino);
	}
	...
	u64 ino, unsigned long pi_addr)
{
	...
	if(ino < NOVA_NORMAL_INODE_START)
	{
		...
		if(ino != pi->nova_ino)
		{
			printk("nova_get_inode_by_ino_or_pi_addr 1 fatel error original get ino is %llu , pi->nova_ino is %llu \n", \
				ino, pi->nova_ino);

			pi = nova_get_inode_by_ino(sb, ino);
			if(pi == NULL)
				goto WWB_GET;
			if(ino != pi->nova_ino)
			{
				printk("nova_get_inode_by_ino_or_pi_addr 2 fatel error original get ino is %llu , pi->nova_ino is %llu \n", \
					ino, pi->nova_ino);
			}
		}
		return pi;
...
/* wwb add 2020-6-28 */
	inode_page_count_increase_by_vir((unsigned long)pi);
/* end 2020-6-28 */
...
/* 重写nova_update_tail */
static inline void nova_update_tail_wwb(struct super_block * sb, struct nova_inode *pi, u64 new_tail)
{
	timing_t update_time;
	...
	PERSISTENT_BARRIER();
	mutex_lock(nova_get_specific_inode_page_lock(pi->nova_ino));
	if(pi->nova_ino > NOVA_NORMAL_INODE_START)
