...
/* wwb add 2020-6-29 */
	unsigned long * inode_page_mapping_table = NULL;
	struct mutex * inode_page_lock;
	//mapping table全局锁，暂时不使用
	//mutex_init(&mapping_table_lock);

	sbi->inode_page_mapping_table_locks = kzalloc(sbi->cpus * sizeof(struct mutex), GFP_KERNEL);
/* end 2020-6-29 */
...
/* wwb add 2020-6-28 修改inode table为4K */
	pi->i_blk_type = NOVA_BLOCK_TYPE_4K;

//分配inode mapping table
	nova_alloc_block_inode_page_mapping_table(sb);
	nova_alloc_block_inode_page_locks(sb);
	//初始化自旋锁
	for(i = 0; i < sbi->cpus; i++)
	{
		mutex_init(&sbi->inode_page_mapping_table_locks[i]);
	}
/* end 2020-6-28 */
	...
	for (i = 0; i < sbi->cpus; i++) {
		...
		/* wwb add 2020-6-28 修改inode table为4K */
		/* 
		 * blocknr以4k为单位，block以字节为单位，相当于对应于起始地址的偏移
		 */
		//mutex_lock(&mapping_table_lock);

		mutex_lock(&sbi->inode_page_mapping_table_locks[i]);
		...
		{
			/* 2020-7-5 */
			mutex_init(&inode_page_lock[j]);
			inode_page_mapping_table[j] = 0;
		}
		
		block = nova_get_block_off(sb, blocknr, NOVA_BLOCK_TYPE_4K);

		inode_page_mapping_table[0] = block;
		//mutex_unlock(&mapping_table_lock);
		mutex_unlock(&sbi->inode_page_mapping_table_locks[i]);
		/* end 2020-6-28 */
	...
	u64 *pi_addr, int extendable)
{
	...
	//DEFINE_SPINLOCK(mapping_table_lock);
	...
	//log_head里面拿的就是inode table的首地址
	//curr = inode_table->log_head;
	curr = inode_page_mapping_table[0];
	...
	for (i = 0; i < superpage_count; i++) {
		...
		curr = inode_page_mapping_table[i+1];
		if (curr == 0) {
			if (extendable == 0)
			{
				printk("%s : extendable == 0 \n",__func__);
				return -EINVAL;
			}
			...
			if (allocated != 1)
			{
				printk("%s : allocated != 1 \n",__func__);
				return allocated;
			}

			curr = nova_get_block_off(sb, blocknr, NOVA_BLOCK_TYPE_4K);

			//spin_lock(&mapping_table_lock);
			inode_page_mapping_table[i + 1] = curr;
			//spin_unlock(&mapping_table_lock);

			DEBUG("Enter this condition \n");
		}
		curr = inode_page_mapping_table[i+1];
	}

	/* 这一句可能会导致bug */
	//printk("WWB: superpage_count is %d \n",superpage_count);
...
/* wwd add 2020-6-30 */
	...
	{
		printk("nova_delete_file_tree: inode didnt match sih->ino is %lu, pi->nova_ino is %lu \n",sih->ino,pi->nova_ino);
	}
/* end 2020-6-30 */
	...
	if (free_blocknr) {
		/* wwb */
		//printk("nova_delete_file_tree: free_blocknr is %lu \n", free_blocknr);
	...
	bool free)
{
	...
	/* wwb 07-05 */
	pi = nova_get_inode_by_ino_or_pi_addr(sb, pi->nova_ino, sih->pi_addr);
...
void nova_evict_inode(struct inode *inode)
{
	...
	if (!inode->i_nlink && !is_bad_inode(inode)) {
		...
		case S_IFREG:
			...
			/* wwb */
			//printk("S_IFREG: nova_evict_inode: last_blocknr is %lu \n", last_blocknr);
			break;
		case S_IFDIR:
		...
		case S_IFLNK:
			...
			freed = nova_delete_file_tree(sb, sih, 0, 0,
							true, true);
			/* wwb */
			//printk("S_IFLNK: nova_evict_inode: last_blocknr is %lu \n", last_blocknr);
			break;
		default:
...
u64 nova_new_nova_inode(struct super_block *sb, u64 *pi_addr)
{
	...
	/*
     * nova_new_nova_inode获取的inode的偏移同样是通过
     * nova_get_inode_address来进行的
     * 此时创建的方式是以可扩展的方式创建
	 */
	...
	size_t size, dev_t rdev, const struct qstr *qstr)
{
	...
	if(pi == NULL)
		printk("WWB: nova_new_vfs_inode pi is null \n");
	...
	/*2020-07-09*/
	mutex_lock(nova_get_specific_inode_page_lock(ino));
	if(pi->nova_ino > 1)
		pi = nova_get_inode_by_ino_or_pi_addr(sb, ino, 0);
	...
	/*2020-07-09*/
	mutex_unlock(nova_get_specific_inode_page_lock(ino));
...
int nova_notify_change(struct dentry *dentry, struct iattr *attr)
{
	...
	nova_update_tail_wwb(sb, pi, new_tail);
	...
	unsigned long blocks, unsigned long checked_pages)
{
	...
	while (curr_p != pi->log_tail) {
		...
		if (!ret) {
			...
			/* wwb add 2020-6-27 */
				u64 block_nums = get_block_number_by_virt(sb,(unsigned long)nova_get_block(sb, new_curr));
				int w_i = 0;
				for(w_i = 0; w_i < length / 4096; w_i++)
					log_block_write_count_increase(block_nums + w_i);
				//printk("WWB: length /4096 is %d nova_inode_log_thorough_gc block number is %lu \n",(length / 4096), block_nums);
			/* wwb end 2020-6-27 */
	...
	u64 curr_tail, u64 new_block, int num_pages)
{
	...
	/* curr指向当前nova_inode的log_head,curr为偏移 */
	curr = pi->log_head;
	sih->valid_bytes = 0;
	...
	while (1) {
		...
		if (curr_page_invalid(sb, pi, sih, curr)) {
			...
			/* 
			 * 如果无效的是log_head,设置first_need_free标志，
			 * 然后将last_page指向curr_page 
			 */
			if (curr == pi->log_head) {
	...
	struct nova_inode_info_header *sih, u64 tail, size_t size, int *extended)
{
	...
	/* wwb add 2021-01-27 */
	if(sih != 0 && pi->nova_ino > NOVA_NORMAL_INODE_START)
		pi = nova_get_inode_by_ino_or_pi_addr(sb, pi->nova_ino, sih->pi_addr);
...
/* wwb add 2020-6-27 */
	u64 block_nums = get_block_number_by_virt(sb,(unsigned long)entry);
