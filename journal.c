...
//	nova_print_lite_transaction(dram_entry1);
	...
	journal_block_write_count_increase(block_nums);
	//block_write_count[block_nums]++;
	//printk("WWB: block_write_count[%lu] is %lu\n", block_nums, block_write_count[block_nums]);
	//printk("WWB: size is %d nova_create_lite_transaction block number is %lu \n",size, block_nums);
	/* wwb end 2020-6-27 */
...
/* wwb add 2020-7-1 */
	/*
	printk("nova_commit_lite_transaction: pair->journal_head is %lu; pair->journal->tail is %lu ; tail is % lu\n", \
		pair->journal_head, pair->journal_tail, tail);
	*/
	active_migrate_journal_page(sb,pair,pair->journal_head);
/* end 2020-7-1 */
}
...
int nova_lite_journal_hard_init(struct super_block *sb)
{
	...
	for (i = 0; i < sbi->cpus; i++) {
		...
		/* wwb */
		//printk("nova_lite_journal_hard_init blocknr is %lu; block is %lu\n",blocknr, block);
