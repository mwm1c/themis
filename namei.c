	...
	struct nova_inode *pi, struct nova_inode *pidir, u64 pidir_tail)
{
	...
	inode_page_count_increase(sb, (unsigned long)pidir);
			...
			bool excl)
{
	...
	/*
	 * 在nova_new_nova_inode里面获取nova_inode的偏移pi_addr;
	 */
			...
			struct inode *new_dir, struct dentry *new_dentry)
{
	...
	/*713*/
	inode_page_count_increase(sb, (unsigned long)old_pi);
	
	old_pidir->log_tail = old_tail;
	nova_flush_buffer(&old_pidir->log_tail, CACHELINE_SIZE, 0);

	/*713*/
	inode_page_count_increase(sb, (unsigned long)old_pidir);
	...
	if (old_pidir != new_pidir) {
		...
		/*713*/
		inode_page_count_increase(sb, (unsigned long)new_pidir);
	}
